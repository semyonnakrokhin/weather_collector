...
class StorageServiceManager:
    """StorageServiceManager manages a collection of AbstractStorageService instances."""

    def __init__(self,
                 all_storage_services: List[AbstractStorageService],
                 selected_storage_services: List[str]):
        """
        Initializes the manager with a list of all storage services and the selected storage services.

        Parameters:
            all_storage_services (List[AbstractStorageService]): The list of all available storage services.
            selected_storage_services (List[str]): The list of designated storage services to be managed.
        """
    ...
    @staticmethod
    def validate_designations(selected_storage_services: List[str]) -> Optional[NoReturn]:
        """
        Validates the selected storage services against the allowed designations.

        Parameters:
            selected_storage_services (List[str]): The list of storage service designations to be validated.
        """
    ...
    def get_selected_storage_services(self) -> List[AbstractStorageService]:
        """
        Returns a list of AbstractStorageService instances corresponding to the selected services.

        Returns:
            List[AbstractStorageService]: The list of selected storage service instances.
        """
        res = []
